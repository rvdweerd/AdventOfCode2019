...
namespace Day21
{
	...
	{
	public:
		IntCode_char(std::string fileName)
		{
		...
		void Run()
		{
			...
			for (code_index = 0; (int)runCodeVec[code_index] != 99; code_index += increment)
			{
				...
				else if (opcode == 3) // input
				{
					...
					writeVal<int>(code_index + 1, C, ch);
					increment = 2;
				}
				else if (opcode == 4) // output
				{
					char val1 = readVal<int>(code_index + 1, C);
					std::cout << val1;// << ",";
					increment = 2;
				}
		...
		int Run(std::string inputInstructions)
		{
			...
			for (code_index = 0; (int)runCodeVec[code_index] != 99; code_index += increment)
			{
				...
				{
					int val1 = readVal<int>(code_index + 1, C);
					if (val1 < 255) std::cout << (char)val1;// << "(" << val1 << ")";
					else std::cout << val1;
					increment = 2;
					//return val1;
				}
